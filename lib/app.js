...
function drawScene() {
		...
		var traslationDrone = [1,0,0,droneX,
							   0,1,0,droneY,
							   0,0,1,droneZ,
							   0,0,0,1];
		//rotation of droneAngle around the y axis
		var rotationDrone = [Math.cos(dronedir2),	0,	Math.sin(dronedir2),	0,
							 0,					1,	0,					0,
							 -Math.sin(dronedir2),0,	Math.cos(dronedir2),	0,
							 0,					0,	0,					1];
		//computing world matrix
		var dvecmat = utils.multiplyMatrices(traslationDrone,rotationDrone);
...
//			angle     = psi/Math.PI*180;
		...
		// draws the request
		gl.bindBuffer(gl.ARRAY_BUFFER, itBoxMesh.vertexBuffer);
		gl.vertexAttribPointer(program.vertexPositionAttribute, itBoxMesh.vertexBuffer.itemSize, gl.FLOAT, false, 0, 0);
	    /*gl.bindBuffer(gl.ARRAY_BUFFER, itBoxMesh.textureBuffer);
	    gl.vertexAttribPointer(program.textureCoordAttribute, itBoxMesh.textureBuffer.itemSize, gl.FLOAT, false, 0, 0);*/
		
		gl.bindBuffer(gl.ARRAY_BUFFER, itBoxMesh.normalBuffer);
		gl.vertexAttribPointer(program.vertexNormalAttribute, itBoxMesh.normalBuffer.itemSize, gl.FLOAT, false, 0, 0);
		 
		gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, itBoxMesh.indexBuffer);		
		...
		gl.uniformMatrix4fv(program.WVPmatrixUniform, gl.FALSE, utils.transposeMatrix(WVPmatrix));		
		gl.uniformMatrix4fv(program.NmatrixUniform, gl.FALSE, utils.transposeMatrix(worldMatrix));
		gl.drawElements(gl.TRIANGLES, itBoxMesh.indexBuffer.numItems, gl.UNSIGNED_SHORT, 0);
